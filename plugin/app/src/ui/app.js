...
export class App {
  ...
  bindGlobalKeys() {
    ...
    const handlers = {
      ...
      'marker.add':          () => { if (!this.timeline.active) return false; const m = this.timeline.addMarker(this.transport.songPos); this.timeline.selectedMarker = m.id; },
      ...
      'loop.toggle':         () => {
        ...
        if (!arr.loop) { const r = this.timeline.range, p = this.transport.songPos; this.store.cycleSet(r ? { a: r.a, b: r.b, on: true } : { a: p, b: p + 8, on: true }); }
        else this.store.cycleSet({ on: !arr.loop.on });
        this.transport.armLoop?.(); this.timeline.dirty = true;
      },
      'loop.fromRange':      () => {
        const arr = this.store.project.arrangement, r = this.timeline.range; if (!arr || !r) return false;
        this.store.cycleSet({ a: r.a, b: r.b, on: true });
        this.transport.armLoop?.(); this.timeline.dirty = true;
      },
